...
static inline void *executorThreadHandler(void *arg) {
    ...
    //累加計時
    static thread_local NnUint totalExecOpTime = 0;
    static thread_local NnUint totalSyncTime = 0;
    ...
    while (true) {
        ...
        if (currentCount == doneCount) {
            if (context->timer != nullptr) {
                ...
                if (step->type == STEP_EXECUTE_OP) {
                    totalExecOpTime += time;
                } else if (step->type == STEP_SYNC_NODES) {
                    totalSyncTime += time;
                }

                const char* stepTypeName = nullptr;
                const char* opName = nullptr;

                if (step->type == STEP_EXECUTE_OP) {
                    stepTypeName = "EXEC_OP";
                    opName = step->opConfig ? step->opConfig->name : "unknown_op";
                } else if (step->type == STEP_SYNC_NODES) {
                    stepTypeName = "SYNC";
                    opName = "sync_nodes";
                } else {
                ...
                printf("[Step %3u] Type: %-9s | Op: %-30s | Time: %8u us\n",
                    currentStepIndex, stepTypeName, opName, time);
    ...
    printf("===== Summary =====\n");
    printf("Total EXEC_OP time: %u us (%.2f ms)\n", totalExecOpTime, totalExecOpTime / 1000.0);
    printf("Total SYNC time   : %u us (%.2f ms)\n", totalSyncTime, totalSyncTime / 1000.0);
    printf("====================\n");
